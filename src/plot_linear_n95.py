...
def plot_linearity_n95():
    ...
    def load_n95_scores(model_name, concept, vector_type, is_remove):
        """
        Load n_components_95 from the saved results.
        """
        ...
        try:
            ...
            for l in layers:
                ...
                if isinstance(val, dict):
                     # EXTRACT n_components_95 HERE
                     scores.append(val.get("n_components_95", 0))
                else:
                     # If old format without this key, return 0 or NaN
                     scores.append(0)
    ...
    for idx, (model_full, model_name) in enumerate(models_order):
        ...
        for concept in concepts:
            ...
            # Random
            layers, scores = load_n95_scores(model_name, concept, "random", is_remove=False)
            if layers:
            ...
            # Concept
            layers, scores = load_n95_scores(model_name, concept, "concept", is_remove=False)
            if layers:
                x_axis = [l / (max_layers - 1) * 100 for l in layers]
            ...
            # Random
            layers, scores = load_n95_scores(model_name, concept, "random", is_remove=True)
            if layers:
                x_axis = [l / (max_layers - 1) * 100 for l in layers]
                ax.plot(x_axis, scores, color=color, marker='o', linestyle=':', 
            ...
            # Concept
            layers, scores = load_n95_scores(model_name, concept, "concept", is_remove=True)
            if layers:
                x_axis = [l / (max_layers - 1) * 100 for l in layers]
                ax.plot(x_axis, scores, color=color, marker='*', linestyle='-.', 
        ...
        if idx == 0:
            ax.set_ylabel("# Components (95% Var)", fontweight='bold')
        ax.set_title(f"{model_name}", fontweight='bold')
        ax.grid(True, linestyle='--', alpha=0.7)
        # Optionally set ymin to 0
