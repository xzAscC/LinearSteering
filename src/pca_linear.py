...
def worker_process(rank, world_size, model_full_name, task_queue, args, max_layers):
    ...
    while True:
        ...
        if not os.path.exists(concept_vectors_path):
            logger.warning(
                f"Rank {rank}: Concept vectors not found for {concept_category_name} in {model_name}. Skipping."
            )
            continue
            ...
            for layer_idx in iterator:
                ...
                for i in range(0, total_prompts, batch_size):
                    ...
                    for alpha in alphas:
                        ...
                        h_flat_batch = hidden_to_flat(
                            h_batch, target_dtype=torch.float32
                        )
                        ...
                        if args.remove_concept_vector:
                            ...
                            h_flat_corrected = hidden_to_flat(
                                h_batch - alpha * steering_vec_device,
                ...
                logger.info(
                    f"Rank {rank}: Linearity for {concept_category_name} in {model_name} at layer {layer_idx}: {stats['mean_score']:.4f} (n95: {stats['mean_n95']:.2f})"
                )
...
def main():
    ...
    for model_full_name, max_layers in models_to_process:
        ...
        if num_gpus > 1:
            ...
            logger.info(
                f"Spawning {num_gpus} workers for {len(all_concepts)} concepts..."
            )
            mp.spawn(
                worker_process,
